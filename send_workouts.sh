...
for i in $(seq 1 "$NUM_WORKOUTS"); do
  ...
  response=$(
    curl -s -X POST "https://alpha-leaderboard-1a2ab4169133.herokuapp.com/api/v1/workouts" \
  ...
  index=$((i - 1))
  curl -s -X PUT "https://alpha-leaderboard-1a2ab4169133.herokuapp.com/api/v1/workouts/$workout_id" \
    -H "Content-Type: application/json; charset=utf-8" \
    -d "{
